...
const clean = (symbol) => symbol ? symbol.split('.')[0] : '';
                  ...
                  {filteredStocks.map((stock) => (
                    <div key={stock.symbol} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 hover:border-gray-600 transition">
                      <div className="flex justify-between items-start mb-3">
                        <div>
                          <p className="font-bold text-sm">{clean(stock.symbol)}</p>
                      ...
                      <p className={`text-sm mb-4 ${stock.change >= 0 ? 'text-green-400' : 'text-red-400'}`}>{stock.changePercent}</p>
                  ...
                  <div className="grid gap-4">
                    {portfolio.holdings.map((h) => (
                      <div key={h.symbol} className="bg-gray-900 border border-gray-800 rounded-2xl p-5 flex justify-between items-center">
                        <div>
                          <p className="font-bold">{clean(h.symbol)}</p>
                    ...
                    {orders.map((o) => (
                      <div key={o._id} className="bg-gray-900 border border-gray-800 rounded-2xl p-4 flex justify-between items-center">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="font-semibold">{clean(o.symbol)}</p>
                        ...
                        <div className="text-right">
                          ...
                          <p className="text-gray-500 text-xs">{o.quantity} x ₹{o.price?.toFixed(2)}</p>
                        </div>
                      </div>
                    ))}
                  </div>
        ...
        <div className="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50 px-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl p-6 w-full max-w-md">
            <div className="flex justify-between items-center mb-5">
              <div>
                <h3 className="text-lg font-bold">{clean(orderModal.symbol)}</h3>
                <p className="text-gray-400 text-sm">₹{orderModal.price?.toFixed(2)}</p>
              </div>
              <button onClick={() => setOrderModal(null)} className="text-gray-500 hover:text-white text-2xl transition">x</button>
            </div>
            ...
            <button onClick={placeOrder} disabled={placing}
              ...
              {placing ? 'Placing...' : `${orderForm.type} ${clean(orderModal.symbol)}`}
            </button>
          </div>
        </div>
      )}
